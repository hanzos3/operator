...
func (c *Controller) checkAndCreatePrometheusAddlConfig(ctx context.Context, tenant *miniov2.Tenant, accessKey, secretKey string) error {
	...
	minIOScrapeConfigs := make([]configmaps.ScrapeConfig, len(promCfg.ScrapeConfigs))
	copy(minIOScrapeConfigs, promCfg.ScrapeConfigs)

	// If the secret is not found, create the secret
	if k8serrors.IsNotFound(err) {
		klog.Infof("Adding MinIO tenant %s/%s Prometheus scrape config", ns, tenant.Name)
		scrapeCfgYaml, err := yaml.Marshal(&minIOScrapeConfigs)
		if err != nil {
			return err
		}
		...
		// Get all non-MinIO scrape configs, as well as MinIO scrape configs for other tenants
		...
		// Add the expected MinIO scrape configs for the current tenant
		expectedScrapeConfigs = append(expectedScrapeConfigs, minIOScrapeConfigs...)

		// Verify that all expected scrape configs are present in the installed configs
		updateScrapeConfig := false
		if len(scrapeConfigs) != len(expectedScrapeConfigs) {
			updateScrapeConfig = true
		} else {
			// Verify that all MinIO scrape configs for the current tenant
			// are included in the installed configs
			for _, minIOConfig := range minIOScrapeConfigs {
				configMatched := false
				for _, config := range scrapeConfigs {
					if minIOConfig.JobName == config.JobName {
						if config.MetricsPath != minIOConfig.MetricsPath ||
							config.Scheme != minIOConfig.Scheme ||
							!reflect.DeepEqual(config.TLSConfig, minIOConfig.TLSConfig) ||
							!reflect.DeepEqual(config.StaticConfigs, minIOConfig.StaticConfigs) {
							updateScrapeConfig = true
							break
						}
						accKey, err := miniov2.GetAccessKeyFromBearerToken(config.BearerToken, secretKey)
						if err != nil || accKey != accessKey {
							updateScrapeConfig = true
							break
						}
				...
				// Update the installed scrape configs if any MinIO configs for
				// the current tenant are missing
				if !configMatched {
					updateScrapeConfig = true
					break
				}
