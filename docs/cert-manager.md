...
This document explains how to deploy Hanzo S3 Operator and a Hanzo S3 Tenant using certificates generated by [cert-manager](https://cert-manager.io/).
...
An `Issuer` is also created in the `hanzo-s3-operator` namespace. More about services that require TLS certificates
in the `hanzo-s3-operator` namespace are covered below in [Hanzo S3 Operator services with cert-manager](#minio-operator-services-with-cert-manager)..
...
> The main difference is that you must provide the `Issuer` CA certificate to Hanzo S3, instead of the CA's mentioned in this guide.
...
- Kubernetes version `+v1.21`. While cert-manager
  supports [earlier K8s versions](https://cert-manager.io/docs/installation/supported-releases/), Hanzo S3 Operator requires 1.21 or later.
...
# Hanzo S3 Operator services with cert-manager

Hanzo S3 Operator manages the TLS certificate issuing for the services hosted in the `hanzo-s3-operator` namespace. That is the Secure Token Service `sts`.
...
Hanzo S3 STS is a service included with Hanzo S3 Operator that provides Native IAM Authentication for Kubernetes. In essence
this service allows you to control access to your Hanzo S3 tenant from your kubernetes applications without having to explicitly create credentials
for each application. For more information on the Service see the Hanzo S3 docs at https://s3.hanzo.ai/docs/minio/kubernetes/upstream/developers/sts-for-operator.html.
There is also an [STS](https://github.com/hanzos3/operator/blob/master/docs/STS.md) guide in the docs and example client code in
https://github.com/hanzos3/operator/tree/master/examples/kustomization/sts-example.
...
This guide covers how to **disable** the automatic generation of the certificate in Hanzo S3 Operator and issue the certificate using
Cert Manager instead.

### Create hanzo-s3-operator namespace CA Issuer

To create the Certificate Authority (CA) certificate used to issue certificates for services in the `hanzo-s3-operator`
namespace, first create the hanzo-s3-operator namespace

```shell
kubectl create ns hanzo-s3-operator
```

Request a Certificate with `spec.isCA: true` specified. This is our CA for the `hanzo-s3-operator` namespace.
...
kind: Certificate
metadata:
  name: hanzo-s3-operator-ca-certificate
...
```
A new secret with the name `operator-ca-tls` is created in the `hanzo-s3-operator` namespace, this is the CA issuing TLS certificates for the services in the `hanzo-s3-operator` namespace.
...
kind: Issuer
metadata:
  name: hanzo-s3-operator-ca-issuer
  namespace: hanzo-s3-operator
spec:
  ca:
    secretName: operator-ca-tls
...
```shell
sts
sts.hanzo-s3-operator.svc.
sts.hanzo-s3-operator.svc.<cluster domain>
```

> [!IMPORTANT]
> Replace `<cluster domain>` with the actual values for your Hanzo S3 tenant.
...
spec:
  dnsNames:
    - sts
    - sts.hanzo-s3-operator.svc
    - sts.hanzo-s3-operator.svc.cluster.local
  secretName: sts-tls
  issuerRef:
    name: hanzo-s3-operator-ca-issuer
```
...
```
This creates a secret called `sts-tls` in the `hanzo-s3-operator` namespace.
...
When installing the Operator deployment, make sure to set `OPERATOR_STS_AUTO_TLS_ENABLED: off` env variable in the `hanzo-s3-operator` container. This prevents
Hanzo S3 Operator from issuing the certificate for STS and instead wait for you to provide the TLS certificate issued by Cert Manager.
...
A way to make sure that the env variables are properly set is using kustomization to patch the `hanzo-s3-operator` deployment:

```yaml
# hanzo-s3-operator/kustomization.yaml
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization

resources:
- github.com/hanzos3/operator/resources
...
```shell
kubectl apply -k hanzo-s3-operator
```

# Hanzo S3 tenant TLS certificates with cert-manager
...
Request Cert Manager issue a new TLS server certificate for Hanzo S3 that includes the following DNS domains:
...
> convention Hanzo S3 Operator uses when creates certificates with Autocert enabled (`spec.requestAutoCert: true`).
...
## Trust tenant-1 CA in Hanzo S3 Operator

Hanzo S3 Operator can trust as many CA certificates as provided. To do this, create a secret with the prefix `operator-ca-tls-` 
followed by a unique identifier in the `hanzo-s3-operator` namespace.

Hanzo S3 Operator mounts and trusts all certificates issued by the provided CA's. This is required because
Hanzo S3 Operator performs health checks using the */minio/health/cluster* endpoint.

If Operator is not correctly configured to trust the Hanzo S3 Certificate (or its CA), you will see an error message like the following in the Operator Pod logs:

```error
Failed to get cluster health: Get "https://minio.tenant-1.svc.cluster.local/minio/health/cluster":
x509: certificate signed by unknown authority
```
For more details about health checks, see https://s3.hanzo.ai/docs/minio/Kubernetes/upstream/operations/monitoring/healthcheck-probe.html#cluster-write-quorum.

### Create `operator-ca-tls-tenant-1` secret

Copy the Cert Manager generated CA public key (ca.crt) into the `hanzo-s3-operator` namespace. This allows Operator to trust
...
Create the secret:
```sh
kubectl create secret generic operator-ca-tls-tenant-1 --from-file=ca.crt -n hanzo-s3-operator
```
> [!TIP]
