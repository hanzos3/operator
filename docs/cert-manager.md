...
in the `minio-operator` namespace are covered below in [Hanzo S3 Operator services with cert-manager](#minio-operator-services-with-cert-manager)..
...
Hanzo S3 Operator manages the TLS certificate issuing for the services hosted in the `minio-operator` namespace. That is the Secure Token Service `sts`.
