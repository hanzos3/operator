...
function install_cert_manager_tenant() {
    ...
    while ! kubectl get secret tenant-certmanager-ca-tls --namespace tenant-certmanager
    do
...
function setup_sts_bucket() {
  ...
  try wait_for_resource_field_selector $namespace job $condition $selector "600s"
